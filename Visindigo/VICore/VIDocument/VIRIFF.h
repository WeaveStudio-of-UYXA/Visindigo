...
namespace VIDocument {
	class VIPublicAPI VIRIFF : public VIObject
	{
		Q_OBJECT;
		VI_OBJECT;
		friend class VIRIFF;
		...
		_Protected QList<VIRIFF*> SubChunks;
		_Protected ChunkType Type;
		_Protected qint32 ChunkSize;
		_Protected qint32 FormType;
		_Public def_init VIRIFF(VISuper* parent = VI_NULL) :VIObject(parent) {
		}
